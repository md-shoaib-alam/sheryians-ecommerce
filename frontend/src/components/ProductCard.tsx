      ...
      <div className="flex flex-col items-start text-left w-full px-1">
        <Link to={`/product/${id}`} className="block w-full">
            <div className="h-10 md:h-9 flex items-start">
              <h3 className="text-brand-dark font-bold text-sm md:text-lg leading-tight font-syne uppercase tracking-tight line-clamp-2 w-full hover:text-brand-red transition-colors">
                {name}
              </h3>
            </div>
        </Link>
        ...
        <div className="flex items-center gap-1 mb-2 md:mb-1 h-4">
          ...
          <span className="text-brand-dark/30 text-[9px] md:text-[9px] ml-1 font-poppins font-bold tracking-widest">({reviews})</span>
        </div>
        ...
        <button 
          ...
          disabled={isAdded}
          className={`w-full md:w-auto px-4 md:px-8 py-2.5 md:py-2.5 border-2 rounded-full font-extrabold transition-all duration-300 uppercase tracking-widest text-[11px] md:text-sm font-poppins whitespace-nowrap shadow-sm flex items-center justify-center gap-2 active:scale-95
            ${isAdded 
