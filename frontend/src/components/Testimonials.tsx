      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testi, index) => (
            <div key={index} className="bg-white p-10 rounded-2xl shadow-soft border border-primary/5 hover:border-accent/40 transition-all duration-300 relative group">
                <Quote className="absolute top-10 right-10 w-12 h-12 text-secondary opacity-20 group-hover:scale-125 transition-transform" />
              <div className="flex gap-1 mb-6">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className={`w-4 h-4 ${i < testi.rating ? 'fill-accent text-accent' : 'text-primary/20'}`} />
                ))}
              </div>
              <p className="text-primary/80 mb-8 italic text-lg font-serif">"{testi.comment}"</p>
