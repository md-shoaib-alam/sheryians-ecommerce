...
router.post('/recipes', async (req: Request, res: Response) => {
  const { title, thumbnail, channel, type, link } = req.body
  ...
  try {
    const recipe = await prisma.recipe.create({
      data: { title, thumbnail: thumbnail || '', channel: channel || 'Shriyans', type, link },
    })
...
// Admin: PATCH /api/admin/recipes/:id
router.patch('/recipes/:id', async (req: Request, res: Response) => {
  const id = req.params.id as string
  const { title, thumbnail, channel, type, link, isActive } = req.body
  const data: Record<string, any> = {}
...
// Admin: DELETE /api/admin/recipes/:id
